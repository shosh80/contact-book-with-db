...
def Delete_Entry():
    if len(select.curselection()) != 0:
        ...
        if result == True:
            cid = contactlist[Selected()]
            sql = "DELETE from customers WHERE name=%s and address=%s"
            mycursor.execute(sql, cid)
            mydb.commit()
            select.delete(ACTIVE, END)
            messagebox.showinfo("Confirmation", "Successfully Deleted The Selected Contact")
    else:
